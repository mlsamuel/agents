"""
improver.py - Eval-driven skill/KB/guideline improvement using Chat Completions.

Generates proposals from failing emails and applies them:
  - skill_edit     → write new version of skill Markdown file
  - kb_entry       → add to knowledge_base.json + re-upload to vector store
...
def generate_proposals(
    ...
    raw = run_simple(client, IMPROVE_SYSTEM, user_msg, IMPROVE_MODEL)
...
def _merge_kb_entries(client: OpenAI, existing: dict, proposed: dict) -> dict:
    user_msg = (
        f"Existing entry:\n{json.dumps(existing, indent=2)}\n\n"
        f"Proposed entry:\n{json.dumps(proposed, indent=2)}\n\n"
        f"Merge these. Keep category={existing['category']!r}."
    )
    raw = run_simple(client, MERGE_KB_SYSTEM, user_msg, MERGE_MODEL,
...
def _merge_guideline_entries(client: OpenAI, existing: dict, proposed: dict) -> dict:
    user_msg = (
        f"Existing entry:\n{json.dumps(existing, indent=2)}\n\n"
        f"Proposed entry:\n{json.dumps(proposed, indent=2)}\n\n"
        f"Merge these. Keep category={existing['category']!r}."
    )
    raw = run_simple(client, MERGE_GUIDELINE_SYSTEM, user_msg, MERGE_MODEL,
                     response_format={"type": "json_object"})
    merged = json.loads(raw)
    merged["category"] = existing["category"]
    merged["topic"] = existing["topic"]
...
def _apply_kb_entry(client: OpenAI, p: dict, vector_store_id: str) -> None:
    entry = p["entry"]
...
def _apply_guideline(client: OpenAI, p: dict, vector_store_id: str) -> None:
    entry = p["entry"]
    ...
    try:
        ...
        print("  Guidelines re-uploaded to vector store")
    except Exception as exc:
