"""
orchestrator_agent.py - Multi-agent pipeline orchestrator (OpenAI Assistants).

Flow per email:
  1. Decompose  — decides which specialist agent(s) are needed (Chat Completions)
  2. Fan out    — specialists run as OpenAI Assistants with function tools + file_search,
                  sequentially (one at a time)
  3. Merge      — if multiple specialists: GPT-4o merges replies into one reply

Specialist agents use:
  - Function tools (in-process Python functions: CRM, tickets, orders, comms)
  - file_search (OpenAI managed vector store for KB retrieval)
...
def _decompose(client: OpenAI, email: dict, classification: dict) -> list[str]:
    ...
    raw = run_simple(client, DECOMPOSE_SYSTEM, user_msg, FAST_MODEL,
...
def _run_one_specialist(
    ...
    """Create, run, and clean up one specialist assistant."""
    ...
    assistant, thread = create_specialist(client, agent_key, skill_content, vector_store_id, skill_tools)
    try:
        with get_tracer().start_as_current_span(f"pipeline.specialist.{agent_key}") as span:
            span.set_attribute("skill_name", skill_name)
            result = run_specialist(client, assistant, thread, email, classification)
...
def _fan_out(
    ...
    """Run specialist assistants sequentially."""
    results = []
...
def _merge(client: OpenAI, email: dict, results: list[SpecialistResult]) -> str:
    ...
    return run_simple(client, MERGE_SYSTEM, user_msg, MODEL)
