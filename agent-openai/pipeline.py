"""
pipeline.py - Main entry point for the agent-openai support pipeline.
...
Usage:
    ...
    python pipeline.py --offset 10 --limit 5
"""
...
def _run_improve(client, section, classification, skill_map, tally, vector_store_id, tracer) -> dict:
...
def _print_summary(output_sections, tally, args, out_path) -> None:
    ...
    if args.save:
        print(f"\n  Saved to {out_path}")
        print("  Results appended to data/pipeline_results.json")


def main() -> None:
    parser = argparse.ArgumentParser(description="agent-openai support pipeline")
