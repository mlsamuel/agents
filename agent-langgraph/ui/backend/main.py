...
async def chat_history(thread_id: str):
    """
    Return prior human/assistant messages for a thread from the checkpointer.

    Reads the saved LangGraph state via graph.aget_state() and filters to only
    HumanMessage / AIMessage entries (skipping ToolMessage and intermediate steps).
    Returns [] for unknown thread_ids.
    """
    ...
    for msg in snapshot.values.get("messages", []):
        ...
        if type_name == "HumanMessage":
            ...
            # Only include messages with actual text (skip pure tool-call steps)
            content = msg.content if isinstance(msg.content, str) else ""
